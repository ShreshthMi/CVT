...
- **P1/extractor fix:** the current extractor never reads `ID` and **dedupes by header comment** — which silently drops the **filler** duplicate. P1 adds `aco_fmaId = ID` and must **not** dedup-by-comment (key by position / `(ID, SECTION)`).
  - **Shipped in VTF-371 (2026-08-25), one half as written and one half not.** The dedup is gone: `IOEXBAcoExtractorService` now emits one row per `CFG_SECTION_OUT` block in file order, and the row carries a **`slot`** ordinal (0-based) as its key.
  - `aco_fmaId = ID` was **deliberately not added**, because it does not discriminate. The rows that the dedup collapsed belong to cards whose two outputs drive the *same* track section, so both slots reference the same FMA — `ID` is equal, and so is `SECTION`. The suggested `(ID, SECTION)` key is therefore unique only on its *position* half. `fma_1_2` does not help either: it is the *referenced FMA's* index, not the slot's, which is why it splits 87/26 across the 113 captured rows rather than evenly. The block ordinal is the only thing that separates the two rows, so that is what was added.
  - Note the collapsed rows were **not** filler duplicates: in the captured package no card lacked `OutputFma2`. All 5 collisions are genuine two-output cards pointing at one section — confirmed as legitimate configuration.
...
- **As-built (BE-06 / VTF-336, `ForwardingDestinationResolver` + `InstancedExpectationEvaluator`):** confirmed the C0391 raw layout — a block's "header" is the **value of the entry whose key equals the block name** (the parser names a block from its first entry). So a `CFG_INT_ID_DEST_NW1` block with first entry `CFG_INT_ID_DEST_NW1 = 32` is socket `0`; `CFG_FWRD_ACD` carries `INT_ID_DEST` (socket) + `CAN_TX_ID` (source DP) as plain entries; `CFG_MY_IP_NW1` carries `MY_IP_NW1_B1..B4`. The resolver reads each home-COM `CFG_FWRD_ACD` entry, resolves socket→dest IP (NW1 `socket+32`, NW2 `socket+48`) → the present COM whose `CFG_MY_IP_NWx` matches; **both networks must resolve to the same present COM**, else `BaselineInconsistentException` → 400 (§3.3). The evaluator set-compares the resolved actual `(CAN_TX_ID, destCom)` members against the expected `{CAN_TX_ID, DEST_COM}` set (missing → FAIL, extra → `UNEXPECTED_OCCURRENCE`).
...
> **As-built (BE-06 / VTF-336, branch `VTF-336` off `682df1c`).** `ConfigValidationV2Service.validate` now consumes both buckets and assembles the summary; `results` is no longer `List.of()`. Occurrence-selection mechanism = **Option B** (raw `ConfigBlock` entries), chosen because the SummaryService extractors are lossy for this purpose (ACO dedups by header comment — dropping the filler — and all extractors display-map values via `ValueMappingService`); Option B reads the raw `ID`/`SECTION`/… entries, matching how the scalar engine compares.

- **Scalar:** `configValidationService.validateParsedFiles(parsedFiles, scalarMap, ctx)`. The §6-era "feed it straight in, no new code" premise was **incomplete**: `DefaultPayloadValidator.validate` enforces `UIInputRequired` presence **and payload types** (it requires `BLOCK_EXISTS` to be a `Boolean`, but the PDQ emits a `String`), which a machine-derived baseline map does not satisfy. Fix (small, Phase-1-safe): added `PayloadValidator.resolve(map)` — payload **resolution without** that UI-input enforcement — and a `ConfigValidationService.validateParsedFiles(files, map, ResolvedPayloadContext)` overload that runs the same dispatch over it. The v2 path resolves leniently (a value the baseline omits leaves its rule dormant); the Phase-1 `validate(map)` path is byte-unchanged. The rules themselves already tolerate the PDQ types (`ProjectBlockCheckRule` uses `asString()`+`parseBoolean`).
- **Instanced:** a dedicated `InstancedExpectationEvaluator` (NOT the registry/`ValidationKey` dispatch — that model has no per-occurrence identity). It groups expectations by `(fileId, block)`, selects the `parsedConfigFile` whose `id == fileId` (absent file → skipped, consistent with the scalar engine validating only uploaded files), and produces `ValidationResult`s directly:
  - **SINGLE** — mirrors `InputMatch` (mandatory) / `OptionalInputMatchOrBlockNotFound` (optional via `defaultValue`: absent block → PASS iff `defaultValue == expectedValue`).
  - **BY_IDENTITY** — **strict set-equality** over the block's occurrences keyed by their identity entries (`linkedId`): expected member missing → FAIL; EXTRA occurrence → FAIL with the new `UNEXPECTED_OCCURRENCE` sentinel; order-independent; per-member value check. (`ValidationStatus` has no `FLAG`, so "extra → FLAG" is a FAIL carrying the sentinel.)
  - **POSITIONAL** — i-th occurrence by `blockIndex`; ACO only; a re-sequenced config FAILs.
  - `CFG_FWRD_ACD` is BY_IDENTITY but its `DEST_COM` is socket-derived → its own path (§5.6).
...
5. **Registry edits (M4) — DEFERRED AGAIN past BE-06 to a follow-up, decided 2026-06-24.** Add `CFG_SWITCH` / `CFG_IP_SWITCH_TIME`; add `CFG_PROJECT_COM` `ProjectBlockCheck` (COMDETAILS); add the dual-FMA rules. **`CFG_AXCNT.BEHAV_INPUT3` is NOT removed** — corrected during BE-06: `OptionalInputMatchOrBlockNotFoundRule` short-circuits on `!payload.isPresent()`, and v2 never puts `BEHAV_INPUT3` in the scalar map, so the registry rule is **dormant in v2** while the instanced `ControlExpectationsBuilder` owns the derived 6/7; removing it would only risk Phase-1. BE-06 left the shared `ValidationConfiguration.json` **untouched** because the firmware `DefaultValue`s are a safety-relevant domain call and the real ADCs **contradict** the old design note: `CFG_SWITCH` defaults `SWITCH_GE=26 / SWITCH_GSF=0 / PRERESET_ACT_TIME=180` (identical commented template across all sample AEB ADCs; block optional), `CFG_SUPERVIS_FMA*` `RESET_TYPE=0 / RESET_DELAY=3` (note said `1`), `CFG_IP_SWITCH_TIME` absent from every sample. The follow-up adds the rules once the defaults are confirmed against the spec (evidence captured in memory `be06-status`). The v2 engine consumes whatever rules exist, so adding them later needs no BE-06 code change.
