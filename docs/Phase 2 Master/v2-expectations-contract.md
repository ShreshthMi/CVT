...
1. **Reshape the carrier records** (the existing `Expectation(String fileName, String block, int instance, String entryKey, String expectedValue)` cannot hold the locked shapes): introduce (a) `fileID` (numeric join, replacing `fileName`), (b) an **optional/composite `linkedID`** replacing `int instance` (must carry `(ID, SECTION)`), (c) a **scalar variant** with no instance. Update `Expectations`, `StubExpectationsPreprocessor`, `ConfigValidationV2Service`. *(Carrier decision D5 "drop fileName" / D6 "0-based ordinals" — see §8.)*
2. **Widen the seam:** `ExpectationsPreprocessor.preprocess(ComAebMap, PdqUploadResponse)` → `preprocess(ValidationInputV2)` (needs `tpfSections` for item-1/RSR_TYPE).
