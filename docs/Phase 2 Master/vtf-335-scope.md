...
| **M4** ⏸️ | **DEFERRED to BE-06 (VTF-336) — decided 2026-06-24.** The registry edits (`CFG_SWITCH`/`CFG_IP_SWITCH_TIME`, `CFG_PROJECT_COM` on COMDETAILS, the 4 dual-FMA rules, and the `CFG_AXCNT.BEHAV_INPUT3` supersede) move to BE-06, because the **rule type** (InputMatch vs InputMatchOrBlockNotFound — real ADCs show these blocks are optional), the **DefaultValue**s, and the **file-scope** are domain decisions best made when the engine actually consumes the scalar bucket. The shared `ValidationConfiguration.json` stays untouched in VTF-335. M4 does **not** block M5 (the instanced bucket emits values regardless of registry rules). | (BE-06) | (BE-06) |
| **M5** 🚧 | **Instanced bucket (§5)** — six derivations, one sub-commit each: **#1 ✅ counting-head `CFG_ZP_FMA1/2`** (DIR_INV, SLCT_TIMEOUT — `CountingHeadExpectationsBuilder`; **every** EvaluatedFma incl. combination tracks, no MAIN filter); #2 supervisor `CFG_SUPERVIS_FMA1/2` (op==null ⇒ no block); #3 ACO `CFG_SECTION_OUT` (positional + filler, aco_fmaId=ID); #4 CHC `CFG_CONTROL` (signed sensor-set adjacency, middle/boundary, E-CHC, derived BEHAV_INPUT3); #5 `CFG_IP_SWITCH` (per-COM, redundantComPresent); #6 `CFG_FWRD_ACD` (Check-B forwarding, one-hop cross-COM SLCT_TIMEOUT=1 set). | preprocessor instanced assembly | per-family unit test vs fixture |

M1→M2→M3 form the spine; **M4 is deferred to BE-06** (2026-06-24); M5 (instanced bucket) is the remaining BE-05 milestone and the bulk.
