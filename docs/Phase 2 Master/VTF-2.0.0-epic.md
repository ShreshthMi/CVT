...
- **Cluster 3 — Track Section.** Four sub-checks: (1) CHC counting-head assignment per sensor/ADC, (2) correct sensors per track section, (3) supervisory track config, (4) ACO/DT IO assignment. Sub-checks 1 and 4 can be ADC-only; 2 and 3 require station layout as external input (input strategy open). Blocks and keys pending AE confirmation. Source: PDQ Config Control Table. Report sheet: Track Section Details.
...
| VTF-340 | BE-10 | Cluster 3: Track Section (four sub-checks; station-layout input strategy to be confirmed) |
